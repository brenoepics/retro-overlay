    ...
    <div v-if="eventAlert.open" class="retro-overlay-event-alert">
      ...
      <div class="retro-overlay-body">
        <div class="retro-overlay-event-message">
          Hello {{ eventAlert.playerUsername }}, {{ eventAlert.staffUsername }} invites you to the event!
        </div>
...
@Component
class EventAlertComponent extends Vue {
  eventAlert = useEventAlertStore()

  joinEvent(roomId: number) {
    Retro.Overlay.communicationManager.sendMessage(new JoinRoomComposer(roomId));
  }

  closeEventAlert() {
    this.eventAlert.setTransitionClass(false);
    setTimeout(() => {
      this.eventAlert.setOpen(false);
    }, 500); // 500ms ya da istediğiniz bir süre
  }
}

export default toNative(EventAlertComponent)
</script>
